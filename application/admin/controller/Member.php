...
//        dump($params);
        ...
        $info = [];
        if($params) $info = Db::name('member')->find($params['id']);
...
//        if(!$validate->check($params)) return DataReturn($validate->getError(),'-100');
        ...
        if(empty($params['id'])){
            $data = Db::name('member')->insert($params);
        }else{
            $data = Db::name('member')->update($params);
        }
    ...
    public function delete(){
        ...
        $params = input('id');
        $data = Db::name('member')->delete($params);
    ...
    public static function StatusUpdate($params = [])
    {
        ...
        // 数据更新
        if(Db::name('member')->where(['id'=>intval($params['id'])])->update([$params['field']=>intval($params['state'])]))
        {
