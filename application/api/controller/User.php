...
class User extends Common
{
    ...
    public function vcode(){
        ...
        $sms = new \base\Sms();
        $res = $sms->SendCode($phone,$code,MyC('home_sms_supplier_invitation'));

        if($res == true) return DataReturn('发送成功','200');
    }
...
//        dump(cache($param['mobile']));
        if($param['code'] !== cache($param['mobile'])) return DataReturn('验证码错误,请重新输入','400');
        unset($param['code']);
        $data = Db::name('user')->update($param);

        if($data) return DataReturn('提交成功','200');

        return DataReturn('error','400');
