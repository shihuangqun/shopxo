...
class GoodsService
{
     ...
     * @param   [array]          $params [输入参数]
     */
    public static function HomeFloorList($sid = [],$params = [])
    {
        // 商品大分类
        ...
        if(!empty($goods_category))
        {
            ...
            foreach($goods_category as &$v)
            {
                ...
                $goods = self::CategoryGoodsList(['where'=>['gci.category_id'=>$category_ids, 'is_home_recommended'=>1,'store_id' => $sid], 'm'=>0, 'n'=>8, 'field'=>'g.id,g.title,g.title_color,g.images,g.home_recommended_images,g.original_price,g.price,g.min_price,g.max_price,g.inventory,g.buy_min_number,g.buy_max_number,g.store_id']);
                $v['goods'] = $goods['data'];
            }
        }
//        return $goods['data'];
...
//                if(!empty($v['store_id']))
//                {
//                    $v['store_id'] = Db::name('store_location')->where('id',$v['store_id'])->value('name');
//                }
...
//        dump($data);
    ...
    public static function GetAdminIndexWhere($params = [])
    {
        ...
        if(session('admin')['role_id'] == 14){//为供应商
            $store_id = Db::name('store_location')
                ->where('store_id',session('admin')['id'])
                ->value('id');
