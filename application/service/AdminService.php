...
class AdminService
{
    ...
    public static function Login($params = [])
    {
        ...
        // 获取管理员
        $admin = Db::name('Admin')->field('id,username,login_pwd,login_salt,mobile,login_total,role_id,status')->where(['username'=>$params['username']])->find();
        if(empty($admin))
        {
        ...
        //是否开启
        if($admin['status'] == 0) return DataReturn('该账户未启用，请审核管理员审核...','-2');
