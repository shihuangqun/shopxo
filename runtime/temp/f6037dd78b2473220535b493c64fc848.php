<?php /*a:4:{s:86:"C:/phpStudy/PHPTutorial/WWW/shopxo/application/admin/view/default/admin\save_info.html";i:1563161086;s:84:"C:/phpStudy/PHPTutorial/WWW/shopxo/application/admin/view/default/public\header.html";i:1563161086;s:81:"C:/phpStudy/PHPTutorial/WWW/shopxo/application/admin/view/default/lib\gender.html";i:1563161086;s:84:"C:/phpStudy/PHPTutorial/WWW/shopxo/application/admin/view/default/public\footer.html";i:1563161086;}*/ ?>
<!DOCTYPE html>
<html>
<head>
...
<div class="content-right">
	<div class="content">
		<!-- form start -->
		<form class="am-form form-validation view-save" action="<?php echo MyUrl('admin/admin/save'); ?>" method="POST" request-type="ajax-url" request-value="<?php echo MyUrl('admin/admin/index'); ?>">
			<legend>
				<span class="fs-16">
				...
				<a href="<?php echo MyUrl('admin/admin/index'); ?>" class="fr fs-14 m-t-5 am-icon-mail-reply"> 返回</a>
			</legend>
			<div class="am-form-group">
				<label>用户名</label>
				<input type="text" placeholder="用户名" name="username" pattern="<?php echo lang('common_regex_username'); ?>" data-validation-message="用户名格式 5~18 个字符（可以是字母数字下划线）" class="am-radius" <?php if(!empty($data)): ?>value="<?php echo htmlentities($data['username']); ?>" disabled<?php endif; ?> required />
			</div>
			<div class="am-form-group">
				<label>登录密码</label>
				<input type="password" placeholder="登录密码" name="login_pwd" pattern="<?php echo lang('common_regex_pwd'); ?>"  data-validation-message="密码格式 6~18 个字符" class="am-radius" <?php if(empty($data)): ?>required<?php endif; ?> />
			</div>
			<div class="am-form-group">
				<label>手机号码</label>
				<input type="text" placeholder="手机号码" name="mobile" pattern="<?php echo lang('common_regex_mobile'); ?>" data-validation-message="手机号码格式错误" class="am-radius" <?php if(!empty($data)): ?>value="<?php echo htmlentities($data['mobile']); ?>"<?php endif; ?> />
			</div>
			<?php if(!isset($data['id']) or $admin['id'] != $data['id']): ?>
				<div class="am-form-group">
					<label>权限组</label>
					<select  class="am-radius chosen-select c-p" name="role_id" data-validation-message="请选择所属角色组" required>
						<option value="">可选择...</option>
						<?php foreach($role as $v): ?>
							<option value="<?php echo htmlentities($v['id']); ?>" <?php if(isset($data['role_id']) and $data['role_id'] == $v['id']): ?>selected<?php endif; ?>><?php echo htmlentities($v['name']); ?></option>
						<?php endforeach; ?>
					</select>
				</div>
			<?php endif; ?>
