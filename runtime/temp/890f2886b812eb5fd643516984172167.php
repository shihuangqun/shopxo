...
<div class="am-container article-content">
	...
	<div class="am-u-md-10 am-u-md-push-2 article-right">
		<div class="am-u-sm-12 article-header">
			<h1 class="am-article-title"><?php echo htmlentities($article['title']); ?></h1>
			<p class="am-article-meta">
				<span>发布时间：<?php echo htmlentities($article['add_time_time']); ?></span>
				<span>浏览次数：<?php echo htmlentities($article['access_count']); ?></span>
			</p>
			<hr class="am-article-divider" />
		</div>
		<div class="am-article"><?php echo $article['content']; ?></div>
	</div>
				...
				<?php if(!empty($category_list)): ?>
					<section data-am-widget="accordion" class="am-accordion am-accordion-gapped" data-am-accordion='{}'>
						<?php foreach($category_list as $v): ?>
							<dl class="am-accordion-item <?php if($article['article_category_id'] == $v['id']): ?>am-active<?php endif; ?>">
								<dt class="am-accordion-title">
									<li class="am-nav-header"><?php echo htmlentities($v['name']); ?></li>
								</dt>
								<dd class="am-accordion-bd am-collapse <?php if($article['article_category_id'] == $v['id']): ?>am-in<?php endif; ?>">
									<div class="am-accordion-content">
										<?php if(!empty($v['items'])): ?>
											<ul class="am-nav">
												<?php foreach($v['items'] as $vs): ?>
													<li>
														<a href="<?php echo htmlentities($vs['url']); ?>" <?php if(!empty($vs['title_color'])): ?>style="color:<?php echo htmlentities($vs['title_color']); ?>;"<?php endif; ?> ><?php echo htmlentities($vs['title']); ?></a>
													</li>
												<?php endforeach; ?>
											</ul>
										<?php endif; ?>
