<?php /*a:3:{s:94:"/www/wwwroot/s.justwaityou1314.com/shopxo/application/admin/view/default/apphomenav/index.html";i:1563161086;s:91:"/www/wwwroot/s.justwaityou1314.com/shopxo/application/admin/view/default/public/header.html";i:1563161086;s:91:"/www/wwwroot/s.justwaityou1314.com/shopxo/application/admin/view/default/public/footer.html";i:1563161086;}*/ ?>
<!DOCTYPE html>
<html>
<head>
...
<div class="content-right">
    <div class="content">
        <!-- form start -->
        <form class="am-form form-validation form-search" method="post" action="<?php echo MyUrl('admin/apphomenav/index'); ?>" request-type="form">
            <div class="thin">
                ...
                <tbody>
                        ...
                        <td>
                            ...
                            <select name="is_need_login" class="chosen-select" data-placeholder="是否需要登录...">
                                ...
                                <?php foreach($common_is_text_list as $v): ?>
                                    <option value="<?php echo htmlentities($v['id']); ?>" <?php if(isset($params['is_need_login']) and $params['is_need_login'] == $v['id']): ?>selected<?php endif; ?>><?php echo htmlentities($v['name']); ?></option>
                                <?php endforeach; ?>
                            </select>
                        </td>
                    </tr>
                    <tr>
                        <td>
                            ...
                            <select name="platform" class="chosen-select" data-placeholder="所属平台...">
                                ...
                                <?php foreach($common_platform_type as $v): if(!in_array($v['value'], ['pc'])): ?>
                    ...
                    <tr>
                        ...
                        <td>
                            ...
                            <a href="<?php echo MyUrl('admin/apphomenav/index'); ?>" class="am-btn am-btn-warning am-radius am-btn-sm reset-submit">清除条件</a>
                        </td>
                    </tr>
                </tbody>
            </table>
        ...
        <div class="am-g m-t-15">
            <a href="<?php echo MyUrl('admin/apphomenav/saveinfo'); ?>" class="am-btn am-btn-secondary am-radius am-btn-xs am-icon-plus"> 新增</a>
        </div>
        <!-- operation end -->
            ...
            <tbody>
                <?php if(!empty($data_list)): foreach($data_list as $v): ?>
                        <tr id="data-list-<?php echo htmlentities($v['id']); ?>" <?php if($v['is_enable'] == 0): ?>class="am-active"<?php endif; ?>>
                            ...
                            <td>
                                <div class="am-circle nav-icon-circle am-vertical-align" <?php if(!empty($v['bg_color'])): ?>style="background-color:<?php echo htmlentities($v['bg_color']); ?>;"<?php endif; ?>>
                                    <?php if(!empty($v['images_url'])): ?>
                                        <a href="<?php echo htmlentities($v['images_url']); ?>" target="_blank">
                                            <img src="<?php echo htmlentities($v['images_url']); ?>" class="am-radius am-vertical-align-middle" width="30" height="30" />
                                        </a>
                                    <?php else: ?>
                            ...
                            <td>
                                <?php echo htmlentities($v['event_value']); if(!empty($v['event_value'])): ?>
                                    <br /><span class="am-badge am-badge-warning am-radius"><?php echo htmlentities($v['event_type_text']); ?></span>
                                <?php endif; ?>
                            </td>
                            <td class="am-hide-sm-only">
                                <a href="javascript:;" class="am-icon-btn am-icon-check submit-state <?php if($v['is_need_login'] == 1): ?>am-success<?php else: ?>am-default<?php endif; ?>" data-url="<?php echo MyUrl('admin/apphomenav/statusupdate'); ?>" data-id="<?php echo htmlentities($v['id']); ?>" data-state="<?php echo htmlentities($v['is_need_login']); ?>" data-field="is_need_login"></a>
                            </td>
                            <td class="am-hide-sm-only">
                                <a href="javascript:;" class="am-icon-btn am-icon-check submit-state <?php if($v['is_enable'] == 1): ?>am-success<?php else: ?>am-default<?php endif; ?>" data-url="<?php echo MyUrl('admin/apphomenav/statusupdate'); ?>" data-id="<?php echo htmlentities($v['id']); ?>" data-state="<?php echo htmlentities($v['is_enable']); ?>" data-field="is_enable" data-is-update-status="1"></a>
                            </td>
                            <td><?php echo htmlentities($v['add_time_time']); ?></td>
                            <td class="view-operation">
                                <a href="<?php echo MyUrl('admin/apphomenav/saveinfo', array_merge($params, ['id'=>$v['id']])); ?>">
                                    <button class="am-btn am-btn-secondary am-btn-xs am-radius am-icon-edit"> 编辑</button>
                                </a>
                                <button class="am-btn am-btn-danger am-btn-xs am-radius am-icon-trash-o submit-delete" data-url="<?php echo MyUrl('admin/apphomenav/delete'); ?>" data-id="<?php echo htmlentities($v['id']); ?>"> 删除</button>
                            </td>
                        </tr>
                    <?php endforeach; ?>
                <?php endif; ?>
